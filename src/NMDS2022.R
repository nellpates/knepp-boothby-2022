...
# Load data
plants <- read.csv("raw-data/all-plants2022.csv") # all data from Knepp and Boothby surveys
...
saveRDS(betaplants, "clean-data/betaplants2022.RDS")

betaplants <- readRDS("clean-data/betaplants2022.RDS")
...
plot(plants_NMDS, display = "sites")
stressplot(plants_NMDS)

orditorp(plants_NMDS, "sites")


colvec <- c("gray0", "gray49")
plot(plants_NMDS)
with(plants,
     points(plants_NMDS,
            display = "sites",
...
check <- as.data.frame(plants_NMDS$points)
